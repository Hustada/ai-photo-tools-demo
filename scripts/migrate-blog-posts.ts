...
// Migration script to move hardcoded blog posts to KV storage

import { BlogPost } from '../api/blog-posts';
...
*Written with passion by CodeCraft | Scout AI Project | Bringing clarity to complex code*`,
    rawContent: '', // Will be the same as content for migration
...
// Function to migrate blog posts to API
export const migrateBlogPosts = async (): Promise<void> => {
  ...
  for (const post of existingBlogPosts) {
    try {
      ...
      if (response.ok) {
        console.log(`✅ Migrated blog post: ${post.metadata.id} - ${post.metadata.title}`);
      } else {
        const error = await response.json();
        console.error(`❌ Failed to migrate ${post.metadata.id}:`, error);
      }
    } catch (error) {
      console.error(`❌ Error migrating ${post.metadata.id}:`, error);
    }
  }
...
// Export the blog posts for direct use if needed
export { existingBlogPosts };