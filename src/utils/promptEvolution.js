...
exports.PromptEvolutionEngine = void 0;
...
class PromptEvolutionEngine {
    ...
    async runEvolutionCycle() {
        ...
        const activePromptKeys = promptKeys.filter(key => !key.includes(':proposed:') && !key.includes(':v'));
        const proposals = [];
        for (const promptId of activePromptKeys) {
            try {
                // Skip if prompt is locked
                if (await (0, kvSchemas_1.isPromptLocked)(promptId)) {
                ...
                if (performance.recommendation === 'evolve') {
                    ...
                    if (!prompt)
                        continue;
                    // Generate mutation
                    const newPrompt = await this.generateMutation(prompt.currentPrompt, performance, prompt.evolution.boundaries);
                    // Validate mutation
                    const validation = await this.validateMutation(newPrompt, prompt.evolution.boundaries);
                    if (validation.passed) {
                        // Propose mutation
                        const proposal = await this.proposeMutation(promptId, prompt, newPrompt, performance, validation);
     ...
     */
    async analyzePerformance(promptId) {
        // Get last 7 days of feedback
        const aggregations = await (0, kvSchemas_1.getFeedbackAggregations)(promptId, 'day', 7);
...
OUTPUT ONLY THE NEW PROMPT TEXT, nothing else.`;
     ...
     */
    async validateMutation(newPrompt, boundaries) {
        const details = [];
        let passed = true;
        // 1. Boundary Check - ensure all boundaries are present
        const boundaryCheck = boundaries.every(boundary => newPrompt.toLowerCase().includes(boundary.toLowerCase()));
        if (!boundaryCheck) {
            passed = false;
     ...
     */
    async runRegressionTests(prompt) {
     ...
     */
    async proposeMutation(promptId, originalPrompt, proposedPrompt, performance, validation) {
        const proposal = {
            promptId,
        ...
        await kv_1.kv.set(proposalKey, proposal, { ex: 30 * 24 * 60 * 60 }); // 30 days TTL
     ...
     */
    async sendNotification(proposal) {
...
exports.PromptEvolutionEngine = PromptEvolutionEngine;
